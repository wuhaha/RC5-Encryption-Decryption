...
file = open('C:\\Users\\me\\rc5.txt','w')

for i in range(5):
	a = bitop.bitop(0x11765029-i,32)
	file.write('a = ' +str(hex(a.bitcut()))+'\n')
	for t in range(5):
		...
		a.number += S[0]
		bitem.number += S[1]
		file.write('b = ' +str(hex(bitem.number))+'\n')
		for j in range(1,13):
			file.write('dout = '+'{0:8x}'.format(a.bitcut()) + '{0:8x}'.format(bitem.bitcut())+'\n')
